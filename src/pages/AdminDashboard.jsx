...
function KPIPanel({ session, profile }) {
  ...
  const refreshKPIs = async (showSpinner = false) => {
    ...
    try {
      ...
      if (showSpinner) toast.error('Failed to load KPI data')
    } finally {
  ...
  if (loading) return (
    <div className="loading-container" style={{ minHeight: 300 }}>
      <div className="spinner" /><span>Loading KPI data...</span>
    </div>
  )
  ...
  return (
    <div style={{ padding: '0 0 40px' }}>
      ...
      <div className="page-header" style={{ marginBottom: 24 }}>
        <div>
          <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <BarChart3 size={26} style={{ color: '#f37a23' }} /> Team KPI Overview
          </h1>
          <p className="page-subtitle">Real-time performance metrics for all CRM users under your account.</p>
        </div>
        <button
          onClick={fetchKPIs}
      ...
      <div className="dashboard-grid" style={{ marginBottom: 28 }}>
        {[
          { label: 'Total Leads Created', value: totals.leads, icon: <UserSquare2 size={20} />, color: '#3b82f6' },
          { label: 'Customers', value: totals.customers, icon: <Users size={20} />, color: '#ec4899' },
          { label: 'Deals Won (Count)', value: totals.dealsCount, icon: <Target size={20} />, color: '#10b981' },
          { label: 'Deals Won (Value)', value: `${profile?.currency || '$'}${totals.dealsValue.toLocaleString()}`, icon: <Briefcase size={20} />, color: '#84cc16' },
          { label: 'Tasks Completed', value: totals.tasks, icon: <CheckCircle size={20} />, color: '#8b5cf6' },
          { label: 'Tickets Resolved', value: totals.tickets, icon: <Ticket size={20} />, color: '#f59e0b' },
          { label: 'Active Team Members', value: users.length, icon: <Shield size={20} />, color: '#f37a23' },
        ].map((s, i) => (
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        {filtered.length === 0 ? (
          <div className="empty-state" style={{ padding: 48 }}>
            ...
            <p>No CRM users found. Create users from the User Management tab.</p>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['User', 'Role', 'Leads', 'Customers', 'Deals Won', 'Deals Value', 'Tasks Done', 'Tickets Resolved', 'Last Active'].map(h => (
...
function UserManagementPanel({ session, profile, onUserCreated }) {
  ...
  const handleDeleteUser = async (userId, userEmail) => {
    if (!window.confirm(`Are you sure you want to remove ${userEmail || 'this user'}?`)) return
  ...
  const handleCreate = async (e) => {
    ...
    if (inputEmail === adminEmail || inputEmail === adminProfileEmail) {
      setInlineError("This username is already in use. Please choose a different one.")
      return
    }
    ...
    try {
      ...
      toast.success(`User "${form.name}" created! They will receive a confirmation email.`, { id: toastId })
  ...
  return (
    <div style={{ padding: '0 0 40px' }}>
      <div className="page-header" style={{ marginBottom: 28 }}>
        <div>
          <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <UserPlus size={26} style={{ color: '#f37a23' }} /> User Management
          </h1>
          <p className="page-subtitle">Create and manage CRM users for <strong>{companyName}</strong>.</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: 24, alignItems: 'start' }}>
        ...
        <div className="card" style={{ padding: 28 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
            <UserPlus size={18} style={{ color: '#f37a23' }} /> Create New User
          </h3>
          <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div className="form-group">
              ...
              <select className="form-input" value={form.role} onChange={e => setForm(p => ({ ...p, role: e.target.value }))} style={selectStyle}>
                <option value="user">User (Regular CRM Access)</option>
                <option value="admin">Admin (Full Access + User Creation)</option>
              </select>
            </div>
            <button
              type="submit"
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          {loading ? (
            ...
            <div className="empty-state" style={{ padding: 40 }}>
              ...
              <p>No users yet. Create one from the form.</p>
            </div>
          ) : (
            <div>
              {users.map(u => (
  ...
  return (
    <div style={{ padding: '0 0 40px' }}>
      <div className="page-header" style={{ marginBottom: 28 }}>
        <div>
          <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <ClipboardList size={26} style={{ color: '#f37a23' }} /> Team Records
          </h1>
          <p className="page-subtitle">View and manage all records created by your team members.</p>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div className="empty-state" style={{ padding: 60 }}>
            ...
            <p style={{ color: '#64748b' }}>No {activeTab} found matching search filters.</p>
          </div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  <th style={{ padding: '12px 20px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', borderBottom: '1px solid #f1f5f9' }}>Record Details</th>
                  <th style={{ padding: '12px 20px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', borderBottom: '1px solid #f1f5f9' }}>Created By (User)</th>
                  <th style={{ padding: '12px 20px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', borderBottom: '1px solid #f1f5f9' }}>Date Created</th>
                  <th style={{ padding: '12px 20px', textAlign: 'center', fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', borderBottom: '1px solid #f1f5f9' }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(r => {
                  const title = r.name || r.title || r.subject || 'Unnamed Record'
                  ...
                  const creatorEmail = r.owner || 'System / Unassigned'
...
export default function AdminDashboard({ session }) {
  ...
  const [activeSection, setActiveSection] = useState('kpi') // 'kpi' | 'users' | 'crm'
  ...
  // CRM nav items (same as user dashboard)
  const crmNavItems = isB2C ? [
    { path: '', label: 'Dashboard', icon: <LayoutDashboard size={16} /> },
    { path: 'accounts', label: 'Customer Profiles', icon: <Users size={16} /> },
    { path: 'services', label: 'Services', icon: <Package size={16} /> },
    { path: 'leads', label: 'Leads', icon: <UserSquare2 size={16} /> },
    { path: 'opportunities', label: 'Deals', icon: <Briefcase size={16} /> },
    { path: 'invoices', label: 'Invoices', icon: <Quote size={16} /> },
    { path: 'tasks', label: 'Tasks', icon: <Search size={16} /> },
    ...
    { path: 'accounts', label: 'Accounts', icon: <Building2 size={16} /> },
    { path: 'opportunities', label: 'Deals', icon: <Briefcase size={16} /> },
    { path: 'quotes', label: 'Quotes & Proposals', icon: <Quote size={16} /> },
    { path: 'reports', label: 'Reports', icon: <BarChart3 size={16} /> },
    { path: 'tickets', label: 'Tickets', icon: <Ticket size={16} /> },
    { path: 'tasks', label: 'Tasks', icon: <Settings size={16} /> },
    { path: 'settings', label: 'Settings', icon: <Settings size={16} /> },
  ]

  const currentCrmPath = location.pathname.replace('/admin/crm', '').replace(/^\//, '')
    ...
    <div className="app-layout">
      ...
      <aside className="sidebar" style={{ overflowY: 'auto' }}>
        ...
        <div style={{ margin: '4px 0 16px', padding: '6px 12px', background: 'linear-gradient(135deg,#fff7ed,#fef3c7)', border: '1.5px solid #fed7aa', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <div>
            <div style={{ fontSize: 10, fontWeight: 800, color: '#f37a23', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Admin Panel</div>
        ...
        <nav className="sidebar-nav">
          <button
            className={`nav-item ${activeSection === 'kpi' && !location.pathname.includes('/crm') ? 'active' : ''}`}
            onClick={() => { setActiveSection('kpi'); navigate('/admin') }}
            style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}
          >
            <span className="nav-icon"><BarChart3 size={18} /></span> KPI Dashboard
          </button>
          <button
            className={`nav-item ${activeSection === 'users' && !location.pathname.includes('/crm') ? 'active' : ''}`}
            onClick={() => { setActiveSection('users'); navigate('/admin') }}
            style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}
          >
            <span className="nav-icon"><Users size={18} /></span> User Management
          </button>
          <button
            className={`nav-item ${activeSection === 'team_records' && !location.pathname.includes('/crm') ? 'active' : ''}`}
            onClick={() => { setActiveSection('team_records'); navigate('/admin') }}
            style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%' }}
          >
            <span className="nav-icon"><ClipboardList size={18} /></span> Team Records
          </button>
        </nav>
        ...
        <nav className="sidebar-nav">
          {crmNavItems.map(item => (
            <button
              key={item.path}
              className={`nav-item ${location.pathname.includes('/crm') && currentCrmPath === item.path ? 'active' : ''}`}
              onClick={() => {
                setActiveSection('crm')
                navigate(`/admin/crm${item.path ? '/' + item.path : ''}`)
              }}
          ...
          <div style={{ padding: '12px', background: 'var(--bg-card)', border: '1.5px solid transparent', borderRadius: 'var(--radius-md)', marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ overflow: 'hidden', flex: 1 }}>
                ...
                <div style={{ fontSize: 10, color: '#f37a23', fontWeight: 700 }}>● Administrator</div>
              </div>
            </div>
          </div>
      ...
      <main className="main-content">
        ...
        <div style={{ padding: '32px' }}>
          ...
          <Routes>
            {/* Admin-only pages at /admin (index) */}
            <Route index element={
              activeSection === 'users'
                ? <UserManagementPanel session={session} profile={profile} onUserCreated={fetchProfile} />
                : activeSection === 'team_records'
                ? <TeamRecordsPanel session={session} profile={profile} />
                : <KPIPanel session={session} profile={profile} />
            } />

            {/* CRM modules at /admin/crm/* — paths must include crm/ prefix */}
            <Route path="crm" element={<DashboardHome session={session} profile={profile} />} />
            <Route path="crm/leads" element={<Leads session={session} profile={profile} />} />
            <Route path="crm/contacts" element={<Contacts session={session} profile={profile} />} />
            <Route path="crm/accounts" element={<Accounts session={session} profile={profile} />} />
            <Route path="crm/services" element={<Services session={session} profile={profile} />} />
            <Route path="crm/opportunities/*" element={<Opportunities session={session} profile={profile} />} />
            <Route path="crm/quotes" element={<Quotes session={session} profile={profile} />} />
            <Route path="crm/invoices" element={<Invoices session={session} profile={profile} />} />
            <Route path="crm/reports" element={<Reports session={session} profile={profile} />} />
            <Route path="crm/tickets" element={<Tickets session={session} profile={profile} />} />
            <Route path="crm/tasks" element={<Tasks session={session} profile={profile} />} />
            <Route path="crm/settings" element={<SettingsPage session={session} profile={profile} />} />
          </Routes>
        </div>
      </main>
    </div>
